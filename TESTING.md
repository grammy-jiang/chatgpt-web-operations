...
| consistency | T0 | `SKILL.md`'s command table matches `scripts/*.py`; every endpoint constant in the code appears in `references/endpoint-discovery.md`; no code outside `chatgpt_client.py` reaches into a sender's private members (docstrings and comments excepted) | a command cannot be added without its row |
